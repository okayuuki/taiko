...
def show_analysis(product):
    ...
    kado_df["全品番の入庫数（箱）"] = kado_df["入庫数（箱）"]
    ...
    #for unique_product in hinban_seibishitsu_df['品番_整備室']:
        ...
        lagged_features['投入間口の渋滞判定フラグ'] = (
            (lagged_features['間口A1の充足率'] > 0.99) |
            (lagged_features['間口A2の充足率'] > 0.99) |
            (lagged_features['間口B1の充足率'] > 0.99) |
            (lagged_features['間口B2の充足率'] > 0.99) |
            (lagged_features['間口B3の充足率'] > 0.99) |
            (lagged_features['間口B4の充足率'] > 0.99)
        ).astype(int)
        ...
        #!＜AWS受験後追加＞
        #!　在庫数で割る
        lagged_features["直近の生産数を表したもの"]=lagged_features['出庫かんばん数（t）']#-lagged_features["在庫数（箱）"]
        #lagged_features["直近の生産数を表したもの"]=lagged_features["直近の生産数を表したもの"].rolling(window=8, min_periods=1).mean()
        #lagged_features["在庫数考慮した出庫数_過去"]=lagged_features["在庫数考慮した出庫数_過去"].rolling(window=5, min_periods=1).mean()
        #lagged_features["在庫数考慮した出庫数_過去"]=lagged_features["在庫数考慮した出庫数_過去"]-lagged_features["在庫数考慮した出庫数_過去"].mean()
        #
        hourly_counts_of_kaisyu, _ , _ , _ = calculate_hourly_counts(Timestamp_df, part_number, seibishitsu, "回収日時", start_date, end_date)#回収
        hourly_counts_of_kaisyu = pd.DataFrame(hourly_counts_of_kaisyu, columns=["回収かんばん数（t）"])
        hourly_counts_of_kaisyu = hourly_counts_of_kaisyu.reset_index()
        hourly_counts_of_kaisyu.columns = ['日時','回収かんばん数（t）']
        lagged_features = pd.merge(lagged_features, hourly_counts_of_kaisyu, on='日時', how='left')
        lagged_features["回収かんばん数（t）_時間遅れ"]= lagged_features["回収かんばん数（t）"].shift(int(kanban_kaiten_nissu) * 24 + nonyu_lt).rolling(window=24, min_periods=1).mean()
        # lagged_features["発注数-回収数"]=(lagged_features['発注かんばん数（t）']-lagged_features['回収かんばん数（t）'])/lagged_features["在庫数（箱）"]
        # # 10行前から20行前の移動平均を計算
        # lagged_features["発注数-回収数"]= lagged_features["発注数-回収数"].shift(24*5).rolling(window=24*5, min_periods=1).mean()
        # st.dataframe(lagged_features)
        # st.dataframe(hourly_counts_of_kaisyu)
        #!発注-回収かんばん数計算
        Timestamp_df2 = Timestamp_df[(Timestamp_df['品番'] == part_number) & (Timestamp_df['整備室コード'] == seibishitsu)]
        st.dataframe(Timestamp_df2)
        # 開始日時と終了日時の計算
        start_time = lagged_features['日時'].min()
        end_time = lagged_features['日時'].max()
        #
        # 在庫数を時間ごとに計算する関数
        def calculate_stock_over_time(df, start_time, end_time, freq='H'):
            #start_time = pd.to_datetime(start_time)
            #end_time = pd.to_datetime(end_time)
            df['発注日時'] = pd.to_datetime(df['発注日時'], errors='coerce')
            df['入庫日時'] = pd.to_datetime(df['順立装置入庫日時'], errors='coerce')
            df['出庫日時'] = pd.to_datetime(df['順立装置出庫日時'], errors='coerce')
            df['回収日時'] = pd.to_datetime(df['回収日時'], errors='coerce')
            df['出庫回収LT'] = df['発注回収LT'] - df['発注順立装置出庫LT']

            # 休日（= 土日）が含まれるか判定
            def contains_weekend(start, end):
                if pd.isna(start) or pd.isna(end):
                    return False  # None（NaN）がある場合は False
                
                date_range = pd.date_range(start=start, end=end, freq='D')
                return any(date.weekday() >= 5 for date in date_range)  # 土日が含まれる場合 True

            # 休日時間を設定（休日が含まれる場合は 2、それ以外は 0）
            df["休日時間"] = df.apply(lambda row: 2 if contains_weekend(row["出庫日時"], row["回収日時"]) else 0, axis=1)
            df['出庫回収LT'] = df['出庫回収LT'] - 2

            # 時間リストを生成（freq='H' で1時間間隔）
            time_range = pd.date_range(start=start_time, end=end_time, freq=freq)

            stock_counts = []
            
            for time in time_range:
                stock_count = ((df['発注日時'] <= (time + pd.Timedelta(hours=1))) & (df['発注日時'] > time)).sum()
                #stock_count = ((df['発注日時'] <= time) & ((df['検収日時'] > time) | df['検収日時'].isna())).sum()
                #stock_count = ((df['出庫日時'] <= time) & 
                        #((df['回収日時'] > time) | df['回収日時'].isna())&
                        #(df['出庫回収LT'] > 1)).sum()
                stock_counts.append({'日時': time, 'かんばん数': stock_count})
            
            return pd.DataFrame(stock_counts)
        stock_df = calculate_stock_over_time(Timestamp_df2, start_time, end_time)
        st.dataframe(stock_df)

        # 折れ線グラフの作成
        fig = px.line(stock_df, x='日時', y='かんばん数', title="かんばん数の推移",
                    #labels={'timestamp': '日時', 'kanban_count': 'かんばん数'},
                    markers=True)  # マーカーをつけてデータポイントを明示
        # Streamlit にプロット表示
        st.plotly_chart(fig)

        lagged_features = pd.merge(lagged_features, stock_df, on='日時', how='left')
        #lagged_features['在庫水準（過去の発注数-出庫数）'] = lagged_features['かんばん数'].shift(kanban_kaiten_nissu*24+nonyu_lt)
        lagged_features['在庫水準（過去の発注数-出庫数）'] = (
            lagged_features['かんばん数']
            .shift(int(kanban_kaiten_nissu) * 24 + nonyu_lt)
            .rolling(window=int(24 * 7), min_periods=1)
            .apply(lambda x: np.nansum(x[x != 0]), raw=True)  # 0 を除外して平均を計算
        )
        lagged_features['kari'] = (
            lagged_features['出庫かんばん数（t）']
            .rolling(window=int(24 * 7), min_periods=1)
            .apply(lambda x: np.nansum(x[x != 0]), raw=True)  # 0 を除外して平均を計算
        )
        lagged_features['在庫水準（過去の発注数-出庫数）'] = lagged_features['在庫水準（過去の発注数-出庫数）']-lagged_features['kari']
        st.dataframe(lagged_features)
        #best_range_order
        #!!発注-回収かんばん数計算終了
        #!計画在庫数計算
        lagged_features['日量箱数/h']=lagged_features['日量箱数']/18
        # シミュレーション設定
        H = int(nyuuko)  # 遡る時間
        lagged_features['日時'] = pd.to_datetime(lagged_features['日時'])
        lagged_features['未来在庫数（箱）']=lagged_features['在庫数（箱）']
        lagged_features.loc[lagged_features['入庫予定かんばん数（t）_補正済'] != 0, '便Ave入庫'] = lagged_features["便Ave"]
    
        lagged_features['便Ave入庫'] = lagged_features['便Ave入庫'].fillna(0)
        for i in range(1, len(lagged_features)):
            lagged_features.loc[i, '未来在庫数（箱）'] = lagged_features.loc[i-1, '未来在庫数（箱）'] + \
                lagged_features.loc[i, '便Ave入庫']
            if lagged_features.loc[i, '稼働フラグ'] == 1: 
                lagged_features.loc[i, '未来在庫数（箱）']=lagged_features.loc[i, '未来在庫数（箱）']-lagged_features.loc[i, '日量箱数/h']
            if lagged_features.loc[i, '未来在庫数（箱）'] < lagged_features.loc[i, '設計値MIN']:
                lagged_features.loc[i, '未来在庫数（箱）'] = lagged_features.loc[i, '設計値MIN']

        # 特定の列を選択
        columns_to_display = ['日時', '在庫数（箱）','未来在庫数（箱）','便Ave入庫','日量箱数/h','稼働フラグ']
        # 指定した列のみを表示
        st.dataframe(lagged_features[columns_to_display])
        # 折れ線グラフの作成
        fig = px.line(lagged_features, x='日時', y='未来在庫数（箱）', title='未来在庫数（箱）',
                    markers=True)  # マーカーをつけてデータポイントを明示
        # Streamlit にプロット表示
        st.plotly_chart(fig)
        # 特定の列を選択
        columns_to_display = ['日時','在庫数（箱）','入庫かんばん数（t）','出庫かんばん数（t）','稼働フラグ','入庫予定かんばん数（t）_補正済','西尾東物流センターor部品置き場での滞留かんばん数（t）_時間変化考慮','工場到着後の入庫作業などではない予定外の入庫かんばん数（t）']
        # 指定した列のみを表示
        st.dataframe(lagged_features[columns_to_display])

        def calculate_elapsed_time_since_last_nyuuko(lagged_features):
            """
            入庫からの経過行数を計算（稼働フラグが 0 の行を除外し、前の値で補完）

            Parameters:
            lagged_features (pd.DataFrame): 入庫データを含むデータフレーム

            Returns:
            pd.DataFrame: 入庫からの経過行数を含むデータフレーム
            int: 入庫間隔の中央値（行数）
            """
            
            # 経過行数を記録する列を初期化
            lagged_features['過去の入庫からの経過行数'] = None
            last_dispatch_index = None  # 直近の入庫行のインデックス
            valid_rows_since_last = 0  # 稼働フラグが1の行のみカウント

            # DataFrameの各行を走査
            for index, row in lagged_features.iterrows():
                if last_dispatch_index is not None:
                    if row['稼働フラグ'] == 1:  # 稼働フラグが 1 の場合のみカウント
                        valid_rows_since_last += 1
                    lagged_features.at[index, '過去の入庫からの経過行数'] = valid_rows_since_last  # 前の値で補完
                
                if row['入庫予定かんばん数（t）_補正済'] > 0:
                    last_dispatch_index = index  # 入庫が発生した行を記録
                    valid_rows_since_last = 0  # カウントをリセット

            # 最初の行の '過去の入庫からの経過行数' 列を None にする
            if len(lagged_features) > 0:
                lagged_features.at[0, '過去の入庫からの経過行数'] = None

            # **稼働フラグが 1 & 入庫かんばん数が 1 以上の行のインデックスを取得**
            valid_dispatch_indices = lagged_features[
                (lagged_features['入庫予定かんばん数（t）_補正済'] >= 1) & (lagged_features['稼働フラグ'] == 1)
            ].index

            # **入庫間隔を計算（稼働フラグが 1 の行のみ考慮）**
            dispatch_intervals = valid_dispatch_indices.to_series().diff().dropna()

            # **入庫間隔の中央値を計算**
            median_interval = dispatch_intervals.median()

            # median_interval が NaN の場合は 0 を設定
        ...
        #! 過去の出庫からの経過時間を計算
        lagged_features, median_interval_out = calculate_elapsed_time_since_last_nyuuko(lagged_features)
        st.dataframe(lagged_features)

        def calculate_variable_rolling_mean(df, target_col):
            """
            各行の過去の入庫からの経過行数をウィンドウサイズとして、
            稼働フラグが0の行をスキップしながら移動平均を計算する

            Parameters:
            df (pd.DataFrame): データフレーム
            target_col (str): 移動平均を適用する対象の列

            Returns:
            pd.Series: 計算されたローリング平均
            """
            rolling_means = []

            for i in range(len(df)):
                X = df.at[i, "過去の入庫からの経過行数"]

                # X が None または NaN の場合、デフォルト値 1 を設定
                if pd.isna(X):
                    X = 1
                
                # 直近 X 行を取得（未来のデータは含めない）
                valid_rows = []
                count = 0
                j = i

                while j >= 0 and count < X:
                    if df.at[j, "稼働フラグ"] == 1:  # 稼働フラグが 1 の行のみ考慮
                        valid_rows.append(df.at[j, target_col])
                        count += 1
                    j -= 1

                if len(valid_rows) > 0:
                    #rolling_means.append(sum(valid_rows) / len(valid_rows))
                    rolling_means.append(sum(valid_rows))
                else:
                    rolling_means.append(None)

            return pd.Series(rolling_means, index=df.index)


        #lagged_features["直近の生産数を表したもの"]=lagged_features["直近の生産数を表したもの"]*lagged_features['過去の入庫からの経過行数']
        #lagged_features["直近の生産数を表したもの"]=lagged_features["直近の生産数を表したもの"].rolling(window=24*5, min_periods=1).max()
        # 累積生産数を適用
        #lagged_features["直近の生産数を表したもの"] = calculate_variable_rolling_mean(lagged_features, "直近の生産数を表したもの")
        
        def calculate_median_by_elapsed_days(df):
            """
            '過去の入庫からの経過行数' のユニークな値ごとに
            '直近の生産数を表したもの' の中央値を計算し、新しい列に格納する

            Parameters:
            df (pd.DataFrame): データフレーム

            Returns:
            pd.DataFrame: 新しい列を追加したデータフレーム
            """
            # '過去の入庫からの経過行数' のユニークな値を取得
            unique_values = df["過去の入庫からの経過行数"].dropna().unique()

            # 新しい列の初期化
            df["過去の入庫からの経過行数に応じた基準値"] = None

            # ユニークな値ごとに処理
            for val in unique_values:
                # 該当する行を抽出
                subset = df[df["過去の入庫からの経過行数"] == val]

                # '直近の生産数を表したもの' の中央値を計算
                median_value = subset["直近の生産数を表したもの"].median()

                # 新しい列に中央値を格納
                df.loc[df["過去の入庫からの経過行数"] == val, "過去の入庫からの経過行数に応じた基準値"] = median_value

            return df

        #lagged_features = calculate_median_by_elapsed_days(lagged_features)
        #lagged_features["直近の生産数を表したもの"] = lagged_features["直近の生産数を表したもの"] - lagged_features["過去の入庫からの経過行数に応じた基準値"]

        #!他品番の入庫が優先されているか特徴量
        lagged_features["他品番入庫フラグ"] = ((lagged_features["全品番の入庫数（箱）"] >= 1) & 
                          (lagged_features["西尾東物流センターor部品置き場での滞留かんばん数（t）_時間変化考慮"] >= 1)).astype(int)
    
        # 特定の列を選択
        columns_to_display = ['日時', '稼働フラグ','入庫予定かんばん数（t）','入庫かんばん数（t）','過去の入庫からの経過行数','直近の生産数を表したもの']
        # 指定した列のみを表示
        st.dataframe(lagged_features[columns_to_display])
        ...
        lagged_features['各時間帯の在庫数（箱）_中央値'] = lagged_features.groupby('Hour')['在庫数（箱）'].transform('median')
        #!＜AWS受験後追加＞
        #lagged_features['各時間帯の在庫数（箱）_中央値'] = lagged_features['未来在庫数（箱）']
        #lagged_features['各時間帯の在庫数（箱）_中央値'] = lagged_features.groupby('Hour')['未来在庫数（箱）'].transform('median')

        #! 各時間帯の中央値を引いた新しい列を作成
        lagged_features['在庫数（箱）_中央値からのズレ'] = (
            lagged_features['在庫数（箱）'] - 
            lagged_features['各時間帯の在庫数（箱）_中央値']
        )
        ...
        #!　直近の生産数の移動窓幅を決定する
        #!---------------------------------------------------------------
        # Xを1から24まで変化させたときの相関を計算
        X_values = np.arange(8, (int(kanban_kaiten_nissu) * 24 + nonyu_lt))
        correlations = []

        for X in X_values:
            lagged_features["直近の生産数を表したものtemp"] = (
                lagged_features["直近の生産数を表したもの"]
                .rolling(window=int(X), min_periods=1)
                .apply(lambda x: np.nanmean(x[x != 0]), raw=True)  # 0 を除外して平均を計算
            )
            
            correlation = lagged_features["直近の生産数を表したものtemp"].corr(lagged_features["在庫数（箱）_中央値からのズレ"])
            correlations.append(correlation)

        # DataFrameを作成
        df = pd.DataFrame({"X": X_values, "相関係数": correlations})

        # Streamlit アプリ
        st.title("ウィンドウサイズ X による相関の変化")
        # Plotlyで可視化
        fig = px.line(df, x="X", y="相関係数", markers=True, title="ウィンドウサイズ X による相関の変化")
        st.plotly_chart(fig)

        # 相関が最大となるXを求める
        max_correlation_index = np.argmax(np.abs(correlations))  # 最大相関のインデックス
        max_correlation_X = X_values[max_correlation_index]  # 最大相関のXの値
        max_correlation_value = correlations[max_correlation_index] 
 
        lagged_features["直近の生産数を表したもの"] = (
           lagged_features["直近の生産数を表したもの"]
            .rolling(window=int(max_correlation_X), min_periods=1)
            .apply(lambda x: np.nanmean(x[x != 0]), raw=True)  # 0 を除外して平均を計算
        )

        st.write("最大相関移動窓：",max_correlation_X,"最大相関係数",max_correlation_value)
        ...
        #!＜AWS受験後追加＞
        lagged_features['いつもの在庫数（箱）'] = lagged_features['各時間帯の在庫数（箱）_中央値']
        column = ['日時','いつもの在庫数（箱）']
        ...
        fig = px.line(basezaiko_data, x='日時', y='いつもの在庫数（箱）', title='いつもの在庫数（箱）',
                    markers=True)  # マーカーをつけてデータポイントを明示
        # Streamlit にプロット表示
        st.plotly_chart(fig)
        ...
        def scatter_and_correlation(dataframe, x_col, y_col):
            ...
            #dataframe = dataframe[dataframe['稼働フラグ'] != 0]
        ...
        scatter_and_correlation(
            dataframe=lagged_features,
            x_col='在庫数（箱）_中央値からのズレ',
            y_col='在庫水準（過去の発注数-出庫数）')
        
        scatter_and_correlation(
            dataframe=lagged_features,
            x_col='在庫数（箱）_中央値からのズレ',
            y_col="回収かんばん数（t）_時間遅れ")
        ...
        for i in range(3):
            ...
            delay_No15 = 0
            timelag_No15 = 0
            data[f'No15_投入間口の渋滞判定フラグ（t-{delay_No15}~t-{delay_No15+timelag_No15}）'] = data['投入間口の渋滞判定フラグ']
            ...
            #過去かんばん
            delay_No18 = int(kanban_kaiten_nissu) * 24 + nonyu_lt
            timelag_No18 = 24*7
            data[f'No18_在庫水準（t-{delay_No18}~t-{delay_No18+timelag_No18}）'] = data['在庫水準（過去の発注数-出庫数）']

            delay_No19 = 0
            timelag_No19 = max_correlation_X
            data[f'No19_直近の生産数を表したもの（t-{delay_No19}~t-{delay_No19+timelag_No19}）'] = data["直近の生産数を表したもの"]

            delay_No20 = 0
            timelag_No20 = 0
            data[f'No20_他品番の入庫フラグ（t-{delay_No20}~t-{delay_No20+timelag_No20}）'] = data["他品番入庫フラグ"]

            delay_No21 = int(kanban_kaiten_nissu) * 24 + nonyu_lt
            timelag_No21 = 24
            data[f'No21_回収かんばん数（t-{delay_No21}~t-{delay_No21+timelag_No21}）'] = data["回収かんばん数（t）_時間遅れ"]
            ...
            X = data[[f'No12_発注かんばん数（t-{delay_No12}~t-{delay_No12+timelag_No12}）',
                      ...
                      f'No15_投入間口の渋滞判定フラグ（t-{delay_No15}~t-{delay_No15+timelag_No15}）',
                      #f'No16_仕入先到着or検収乱れ（t-{delay_No16}~t-{delay_No16+timelag_No16}）',
                      #f'No17_計画生産台数（t-{delay_No17}~t-{delay_No17+timelag_No17}）',
                      f'No19_直近の生産数を表したもの（t-{delay_No19}~t-{delay_No19+timelag_No19}）',
                      f'No18_在庫水準（t-{delay_No18}~t-{delay_No18+timelag_No18}）',
                      #f'No20_他品番の入庫フラグ（t-{delay_No20}~t-{delay_No20+timelag_No20}）',
                      #f'No21_回収かんばん数（t-{delay_No21}~t-{delay_No21+timelag_No21}）'
                    ]]
...
def step3(bar_df, df2, selected_datetime, line_df):
    ...
    #!ルールベースで集める要因をつける（No20、No21の作成）
    #Todo 品番名&dataを取り出すために実行、きれいじゃないから要修正
    with open('temp/model_and_data.pkl', 'rb') as file:
        rf_model, rf_model2, rf_model3, X, data, product = pickle.load(file)
    #st.dataframe(data)
    # "No21" を含む列名を抽出
    no20_columns = [col for col in data.columns if "No20" in col][0]
    # "No21" を含む列名を抽出
    no21_columns = [col for col in data.columns if "No21" in col][0]
    #st.write(no20_columns)
    #! df2に説明変数にいれなかった要因を追加する
    df2 = pd.merge(df2, data[["日時",no20_columns, no21_columns]], on='日時', how='left')

    #!平均計算用で必要な列だけ計算したdataを用意する
    no12_columns = [col for col in data.columns if "No12" in col][0]
    no13_columns = [col for col in data.columns if "No13" in col][0]
    no14_columns = [col for col in data.columns if "No14" in col][0]
    no15_columns = [col for col in data.columns if "No15" in col][0]
    no18_columns = [col for col in data.columns if "No18" in col][0]
    no19_columns = [col for col in data.columns if "No19" in col][0]
    no20_columns = [col for col in data.columns if "No20" in col][0]
    no21_columns = [col for col in data.columns if "No21" in col][0]
    datadata = data[[no12_columns,no13_columns,no14_columns,no15_columns,no18_columns,no19_columns,no20_columns,no21_columns]]
    ...
    if not filtered_df1.empty:
        ...
        #! No12のコピー行を作成⇒No20
        # '変数'列の値が 'No13' の行を複製
        duplicate_rows = df1_long[df1_long['変数'].str.contains('No13', na=False)]
        duplicate_rows["変数"]=no20_columns
        #st.dataframe(duplicate_rows)
        # オリジナルのデータフレームに結合
        df1_long = pd.concat([df1_long, duplicate_rows], ignore_index=True)
        #! No13のコピー行を作成⇒No21
        # '変数'列の値が 'No12' の行を複製
        duplicate_rows = df1_long[df1_long['変数'].str.contains('No12', na=False)]
        duplicate_rows["変数"]=no21_columns
        #st.dataframe(duplicate_rows)
        # オリジナルのデータフレームに結合
        df1_long = pd.concat([df1_long, duplicate_rows], ignore_index=True)
        ...
        with tab1:
            ...
            #st.write("確認")
            #st.dataframe(df2_cleaned)

            #! 各要因の値の平均値と中央値を計算
            #average_values = df2_cleaned.mean()
            #median_values = df2_cleaned.median()
            average_values = datadata.mean()
            median_values = datadata.median()
