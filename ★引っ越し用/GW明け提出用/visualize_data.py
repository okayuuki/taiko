...
from get_data import compute_hourly_buhin_zaiko_data_by_all_hinban, compute_hourly_tehai_data_by_hinban
...
def show_abnormal_results( selected_date, selected_time, flag_useDataBase, kojo):
    ...
    # 処理の説明
    st.write("**ステップ０の処理を受け付けました。下限割れor上限越えしている品番をリストアップします。常時上限越え品番は基準の定義や設計値の設定に問題があるかもしれないため、除外しています。**")
    ...
    # Activedata
    #! ↓　手配データを更新するために適当な変数で読み込む。手配ファイルを更新しないと、エラー出る
    time_granularity = '1h'
    # 1Yまたは1Z,2Sの条件でフィルタリングしてから最初の組み合わせを取得
    hinban_info = (zaiko_df[['品番', '整備室コード']]
                .loc[zaiko_df['整備室コード'].isin(['1Y', '1Z','2S'])]  # 1Yまたは1Zでフィルタリング
                .drop_duplicates()
                .iloc[0]
                .values)
    compute_hourly_tehai_data_by_hinban(hinban_info, selected_datetime_start, selected_datetime_end, time_granularity, flag_useDataBase, kojo)
