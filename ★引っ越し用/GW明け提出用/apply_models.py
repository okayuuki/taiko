...
def show_feature_importance( merged_data_df, X, selected_datetime, shap_values):
    ...
    def map_increase_factor_detail(variable):
        if "No1_" in variable:
            return """
                    ...
                    ＜説明＞<br>
                    この特徴量は、ある時点 t における在庫数に影響を及ぼす納入要因として、t - τ 時点に予定されていた納入かんばん数を示すものである。<br>
                    ここで τ（タウ）は、納入から入庫までのリードタイム（納入入庫LT）に相当し、時点 t において在庫数に寄与する納入かんばんの存在量を表現している。<br>
                    つまり、この特徴量は、一時的に在庫数を押し上げる上昇要因を定量化するものである。<br>
                    なお、実際の τ は稼働状況に応じて変動するため、稼働実績を基に動的に算出している。<br>
                    ＜考えられる事象＞<br>
                    ・便Aveより納入かんばん数が多い<br>
                    ・仕入先挽回納入
                    """
            ...
            return """
                    ...
                    ＜説明＞<br>
                    この特徴量は、ある時点 t における、過去数十時間にわたる特定の時間帯（t - τ, t）に対して、<br>
                    1分あたりの平均生産台数を計算したものである。この値は、生産活動における単位時間あたりの生産密度を表現しており、<br>
                    ある時点 t における順立装置内の在庫数に対する短期的な減衰要因として機能する。<br>
                    したがって、この特徴量の値が大きい場合は、在庫数が減少し、小さい場合は在庫数が増加すると考えられる。<br>
                    ＜考えられる異常＞<br>
                    ・ライン停止<br>
                    ・生産変動/計画変更/得意先の需要減
                    """
             ...
             return """
                    ...
                    この特徴量は、ある時点 t において、t - かんばん回転日数以前に発注されたかんばんのうち、<br>
                    順立装置における入庫予定時間 ≦ t ＜ 出庫予定時間を満たすかんばんの数をカウントしたものである。<br>
                    これは、ある時点 t において期待される順立装置内の在庫水準を表現する指標であり、在庫量の土台（ベースライン）を規定するかんばん要因と考えられる。<br>
                    したがって、この特徴量の値が大きい場合は、在庫水準が増加し、小さい場合は在庫水準が減少すると考えられる。<br>
                    なお、本特徴量は、1日単位あるいはそれ以上の比較的長期的なスパンにおける在庫水準の変動を規定する要因として位置付けられる。<br>
                    ＜考えられる事象＞<br>
                    ・過去の生産が多いことで発注増<br>
                    ・臨時かんばんの発行
                    """
            ...
            return """
                    ...
                    この特徴量は、ある時点tの在庫数に影響を与える要因として、通常の入庫予定（=過去の納入予定かんばん数に納入入庫LTを加味して予測される値）では説明できない、異常な入庫かんばん数を示したものである。<br>
                    すなわち、t時点に発生した入庫のうち、事前に予測されていなかった異常な増加分を抽出したもので、部品置き場からの入庫や前倒し入庫といった突発的な在庫増加の要因を捉えるものである。<br>
                    ＜考えられる事象＞<br>
                    ・滞留からの入庫がいつもより多い
                    """
    ...
    def map_decrease_factor_detail(variable):
        if "No1_" in variable:
            ...
            return """
                    ...
                    ＜説明＞<br>
                    この特徴量は、ある時点tにおいて、順立装置入庫予定時間 ≦ tを満たすかんばんの数をカウントしたものである。<br>
                    これは、ある時点 t における西尾東物流Cから部品置き場で滞留している状況を定量的に捉えるものであり、在庫数に対して短期的に減衰方向に影響を及ぼす要因として機能すると考えられる。<br>
                    したがって、この値が大きい場合は在庫も減少しし、少ない場合は在庫も増加すると考えられる。<br>
                    ＜考えられる事象＞<br>
                    ・順立装置の設備停止<br>
    ...
    X = merged_data_df[youin_columns]
    filtered_X = X.loc[filtered_merged_data_df.index[0]]
    filtered_X.name = '要因の値'
    filtered_X.index.name = '変数名'
    # ここで"youin"を削除してインデックスを設定
    filtered_X.index = filtered_X.index.str.replace('youin_', '').str.strip()
    filtered_X.index.name = '変数名'
    #st.dataframe(filtered_X)
    ...
    # ここで"youin"を削除してインデックスを設定
    average_X.index = average_X.index.str.replace('youin_', '').str.strip()
    average_X.index.name = '変数名'
    ...
    # ここで"feature_"を削除してインデックスを設定
    filtered_shap_values.index = filtered_shap_values.index.str.replace('feature_', '').str.strip()
    filtered_shap_values.index.name = '変数名'
    # 寄与度結果の確認
