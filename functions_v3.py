...
#! 途中処理の表示
def display_message(content, role="assistant", place="main"):
    ...
    if place == "main":
        ...
        # メッセージを表示
        for message in messages:
            st.sidebar.chat_message(message["role"]).markdown(message["content"])
...
#! 品番のイベント時間（1時間）ごとのかんばん数を計算する＋検収の場合は納入便や平均納入時間も計算する
def calculate_hourly_counts(df, part_number, seibishitsu, time_col, start_date, end_date):
    ...
    # 実行結果の確認
    display_message(f"**【{time_col}ベース】品番{part_number}_整備室コード{seibishitsu}の所在管理リードタイムデータ（関所別のタイムスタンプデータ）を抽出しました。**")
    ...
    if time_col == '検収日時':
        ...
        delivery_totals.name = '仕入先納入便'#列名（name属性）を変更（これをしないと列名が0になる）
        # イベント時間ごとの平均検収時間を計算
...
def process_shiresakibin_flag(lagged_features, arrival_times_df):
    ...
    #! 仕入先ダイヤフラグを計算できるかの確認
    #st.header("✅以下の仕入先ダイヤがエクセルにあるか検索します")
    display_message(f"**以下の仕入先ダイヤがエクセルにあるか検索します**")
    st.code(f"仕入先名：{lagged_features['仕入先名'].unique()}、発送場所名：{lagged_features['発送場所名'].unique()}、整備室コード：{lagged_features['整備室コード'].unique()}")
    ...
    #! データフレームが空の場合（条件を満たす行をarrival_times_dfがなかった場合）
    ...
    #! データフレームがうまく作成できた場合（条件を満たすarrival_times_dfが存在する場合）
    else:

        # 実行結果の確認
        display_message(f"**一致した仕入先ダイヤを抽出します**")
        st.dataframe(matched_arrival_times_df)

        nonyu_type = matched_arrival_times_df["納入先"].iloc[0]
        
        #todo コラム名変更
        matched_arrival_times_df = matched_arrival_times_df.rename(columns={'受入': '整備室コード'})

        #! arrival_times_dfとlagged_featuresを結合
        lagged_features2 = lagged_features.merge(matched_arrival_times_df, on=['仕入先名','発送場所名','整備室コード'], how='right')#todo

        #! '納入便'列をint型に変換
        # '納入便'を含む列名を見つける、columns_delivery_numは'納入便'を含む列名
        ...
        #! columns_delibery_num列（'納入便'列）の値が0でない場合、columns_delibery_num列に記載された便番号を使用し、
        #! その便に対応する列（例えば「1便_早着」、「2便_早着」など）の値を取得し、「早着」列に設定
    ...
    # lagged_features2に統合済みだけど、分けて渡す
    return lagged_features2, matched_arrival_times_df, nonyu_type
...
def plot_inventory_graph(line_df, y_pred_subset, y_base_subset, Activedata):
    ...
    # タブの作成
    tab1, tab2 = st.tabs(["実績値といつもの値を比較する", "実績値とAI推定値を比較する"])
...
def plot_inventory_graph2(line_df, y_pred_subset, y_base_subset, Activedata, highlight_time):
    ...
    with tab1:
        ...
        for var in line_df.columns[1:]:
            ...
            # highlight_time に対応する y 値を取得
            y_value = line_df.loc[line_df["日時"] == highlight_time, var].iloc[0]
...
def display_shap_contributions(df1_long):
    ...
    def map_factor_positive(variable):
        if variable.startswith("No1_"):
            ...
            return "「発注がある」"
        elif variable.startswith("No11_"):
            return "「予定外の入庫がある」"
        #!new 
        elif variable.startswith("No12_"):
            return """
                    <strong>現在、入庫予定かんばん数が多い</strong><br>
                    ＜説明＞<br>
                    ...
                    ・仕入先挽回納入
                    """
        elif variable.startswith("No13_"):
            return "「None西尾東BC or 部品置き場で滞留しているかんばん数が少ない」"
        elif variable.startswith("No14_"):
            return """
                    <strong>現在、設計外の入庫数が多い</strong><br>
                    ＜説明＞<br>
                    設計外の入庫とは、設計通りの入庫ではないものを表します。<br>
                    ＜考えられる事象＞<br>
                    ・部品置き場などで滞留していた部品を入庫している
                    """
        elif variable.startswith("No15_"):
            return "「None_間口OK」"
        elif variable.startswith("No16_"):
            return "「仕入先便早く到着し普段より早い定期便で工場にモノが届いている」"
        elif variable.startswith("No17_"):
            return "「過去の生産計画が多いため、外れかんばんが多く、発注かんばんが多い」"
        elif variable.startswith("No18_"):
            return """
                    <strong>過去（1週間前程度）の発注かんばん数が多かった</strong><br>
                    ＜説明＞<br>
                    ...
                    ・生産に対して納入かんばんが多かった
                    """
        elif variable.startswith("No19_"):
            return """
                    <strong>直近（現在～1日前まで）の生産数が少ない<br>
                    ＜考えられる異常＞<br>
                    ・ライン停止<br>
                    ・生産変動/計画変更/得意先の需要変化
                    """
        elif variable.startswith("No20_"):
            return "「None他品番の入庫が優先されている」"
        elif variable.startswith("No21_"):
            return """
                    <strong>直近（かんばん回転日数前）の回収かんばん数が多い</strong><br>
                    ＜説明＞<br>
    ...
    def map_factor_negative(variable):
        ...
        #!new 
            ...
            return """
                    <strong>現在、入庫予定かんばん数が少ない</strong><br>
                    ＜説明＞<br>
                    入庫予定かんばん数は「LINKSのデータ」と「仕入先ダイヤのデータ」をもとに以下で計算されています。<br>
                    ①西尾東を経由する部品：現在から5時間程度前の納入予定かんばん数<br>
                    ②直納の部品：現在から1時間程度前の納入予定かんばん数<br>
            ...
            return """
                    <strong>現在、西尾東BCから部品置き場の間で部品が滞留している</strong><br>
                    ＜考えられる事象＞<br>
                    ・順立装置の設備停止<br>
                    ・順立前の部品置き場で部品が残っている<br>
                    ・西尾東BCで部品が残っている/誤転送<br>
                    ・工場ビットの部品OF<br>
                    ・西尾東でもOF<br>
                    ・定期便の乗り遅れ<br>
                    ・台風積雪などによるトラックの遅延<br>
            ...
            return """
                    <strong>現在、設計外の入庫数が少ない</strong><br>
                    ＜説明＞<br>
                    設計外の入庫とは、設計通りの入庫ではないものを表します。<br>
                    ＜考えられる事象＞<br>
                    ・部品置き場などで滞留していた部品を入庫していない
                    """
        elif variable.startswith("No15_"):
            return """
                    <strong>現在、投入間口が一杯で入庫できない</strong><br>
                    ＜考えられる異常＞<br>
                    ・偏った箱種の入庫<br>
                    ・入庫キャパ越え            
                    """
            ...
            return """
                    <strong>過去（1週間程度前）の発注かんばん数が少なかった</strong><br>
                    ＜説明＞<br>
                    在庫推移は時系列で変動しているため、過去の在庫水準が現在の在庫数に寄与していると考えられます。<br>
                    過去の在庫水準を「LINKSのデータ」をもとに以下で計算しています。<br>
                    ・かんばん回転日数前から＋1週間の間の発注かんばん数-回収かんばん数<br>
            ...
            return """
                    <strong>直近（現在～1日前まで）の生産数が多い</strong><br>
                    ＜考えられる異常＞<br>
                    ・生産変動/挽回生産<br>
                    ・計画変更/得意先の需要変化<br>
                    """
            ...
            return """
                    <strong>現在、他品番の入庫が優先されている</strong><br>
                    ＜考えられる事象＞<br>
                    ・先入先出になっていない
                    """
        elif variable.startswith("No21_"):
            return """
                    <strong>過去（かんばん回転日数前）の回収かんばん数が少ない</strong><br>
                    ＜説明＞<br>
                    回収かんばん数が少ないor多いと、発注かんばん数が少ないor多くなる可能性があります。<br>
                    回収かんばん数は「LINKSデータ」をもとに計算しています。<br>
                    ＜考えられる事象＞<br>
    ...
    top_positive = top_positive[~top_positive['要因名'].str.contains(keyword, na=False)]
    ...
    # スタイリングを適用
    def bar_chart_red(s):
        return ['background: linear-gradient(90deg, #FFB99B {}%, transparent 0%)'.format(val) for val in s]
    
    # スタイリングを適用
    def bar_chart_blue(s):
        return ['background: linear-gradient(90deg, #9BD8FF {}%, transparent 0%)'.format(val) for val in s]
    ...
    mapping = {
        ...
        #'稼働率': '過去出庫からの平均稼働率=',
        #'西尾東BC or 部品置き場で滞留': '西尾東BC or 部品置き場の滞留かんばん数=',
        ...
        #'正常な入庫': '入庫かんばん数=',
        #'異常な入庫': '入庫かんばん数='
        "過去（1週間程度前）の発注かんばん数":"過去の発注かんばん合計数-回収かんばん合計数＝",
        "現在、入庫予定かんばん数":"入庫予定かんばん数=",
        "現在、設計外の入庫数が":"設計外の入庫かんばん数=",
        "過去（かんばん回転日数前）の":"直近の回収かんばん数=",
        "現在、西尾東BCから部品置き場の間で":"西尾東BCから部品置き場の間の滞留かんばん数=",
        "直近（現在～1日前まで）":"直近の生産合計数＝",
        "現在、投入間口が一杯":"投入間口の渋滞有無（0：渋滞無し、1：渋滞発生中）=",
        "現在、他品番の入庫が優先されている":"他品番の入庫有無（0：他品番入庫無し、1：他品番入庫有り）＝"
    }
    ...
    if len(top_positive) != 0:
        ...
        styled_positive = top_positive.style.pipe(set_header_color, '#FFB99B')
    ...
    if len(top_negative) != 0:
        ...
        styled_negative = top_negative.style.pipe(set_header_color, '#9BD8FF')
        styled_negative = styled_negative.apply(bar_chart_blue, subset=['寄与度（SHAP値）割合'])
