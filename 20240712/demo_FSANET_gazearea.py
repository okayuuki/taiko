...
def draw_results(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot):
    ...
    if len(detected) > 0:
        for i, (x,y,w,h) in enumerate(detected):
            ...
            # 元画像全体を3分割してハイライト
            # 画像を3分割してハイライト
            highlighted_img = highlight_attention_region_on_split_image(input_img, p_result[0][0])
            img = draw_axis(highlighted_img[yw1:yw2 + 1, xw1:xw2 + 1, :], p_result[0][0], p_result[0][1], p_result[0][2])

            # 注視領域を色分け
            #colored_img = color_attention_region(input_img[yw1:yw2 + 1, xw1:xw2 + 1, :], p_result[0][0])
            #img = draw_axis(colored_img, p_result[0][0], p_result[0][1], p_result[0][2])
...
def main():
    ...
    record_duration = 15  # 録画時間を15秒に設定
