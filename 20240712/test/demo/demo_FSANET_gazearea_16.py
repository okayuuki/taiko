...
def highlight_attention_region_on_split_image(img, yaw, pitch):
    ...
    # 画像を16個の領域に分割（縦4分割、横4分割）
    regions = []
    for i in range(4):
        for j in range(4):
            region = img[int(i * img_h / 4):int((i + 1) * img_h / 4), int(j * img_w / 4):int((j + 1) * img_w / 4)]
            regions.append(region)

    # 頭部方向に応じて対応する領域をハイライト
    if pitch >= 30:  # 上極端
        if yaw <= -30:
            region_to_highlight = regions[3]  # 右上極端
        elif yaw <= -10:
        ...
        if yaw <= -30:
            region_to_highlight = regions[7]  # 右上
        elif yaw <= -10:
            region_to_highlight = regions[6]  # 中央右上
        elif yaw >= 10:
            region_to_highlight = regions[5]  # 上中央
        else:
            region_to_highlight = regions[4]  # 左上
    elif pitch > -10:  # 中央
        if yaw <= -30:
            region_to_highlight = regions[11]  # 右中央
        elif yaw <= -10:
            region_to_highlight = regions[10]  # 中央右
        elif yaw >= 10:
            region_to_highlight = regions[9]  # 中央
        else:
            region_to_highlight = regions[8]  # 中央左
    elif pitch > -30:  # 下
        if yaw <= -30:
            region_to_highlight = regions[15]  # 右下
        elif yaw <= -10:
            region_to_highlight = regions[14]  # 中央右下
        elif yaw >= 10:
            region_to_highlight = regions[13]  # 下中央
        else:
            region_to_highlight = regions[12]  # 左下
    else:  # 下極端
        if yaw <= -30:
            region_to_highlight = regions[15]  # 右下極端
        elif yaw <= -10:
            region_to_highlight = regions[14]  # 右下
        elif yaw >= 10:
            region_to_highlight = regions[13]  # 下中央
        else:
            region_to_highlight = regions[12]  # 左下極端
    ...
    # 16個の領域を再結合して1つの画像に戻す
    first_row = np.hstack((regions[0], regions[1], regions[2], regions[3]))
    second_row = np.hstack((regions[4], regions[5], regions[6], regions[7]))
    third_row = np.hstack((regions[8], regions[9], regions[10], regions[11]))
    fourth_row = np.hstack((regions[12], regions[13], regions[14], regions[15]))
    highlighted_img = np.vstack((first_row, second_row, third_row, fourth_row))
