...
def main(image_path):
    ...
    # 手動で領域を選択
    roi = cv2.selectROI("Select Region", input_img, fromCenter=False, showCrosshair=True)
    x, y, w, h = roi
    detected = [(x, y, w, h)]  # 選択された領域をリスト形式で保存

    faces = np.empty((len(detected), img_size, img_size, 3))
    result_img = draw_results(detected, input_img.copy(), faces, ad, img_size, img_w, img_h, model)

    # 選択された領域を矩形で囲む（表示用）
    cv2.rectangle(input_img, (x, y), (x + w, y + h), (255, 0, 0), 2)

    # 元画像と結果画像を横に結合
    combined_img = concat_images(input_img, result_img)

    # 結果を表示
    cv2.imshow("Select Region and Result", combined_img)
    cv2.imwrite('result_combined.png', combined_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

if __name__ == '__main__':
    image_path = 'test1_workman.png'  # Replace with your image path
    main(image_path)
