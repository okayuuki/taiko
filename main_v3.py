...
def analysis_page():
    ...
    # チャット内容
    messages = [
        {"role": "user", "content": "**この画面では、在庫変動の要因分析を行うことができます。実行する際は左側のサイドバーで各種設定を行ってください**"},
    ]
    # メッセージを表示
    ...
    if submit_button_step0 == True: 
        ...
        # チャット内容
        messages = [
            {"role": "assistant", "content": "**ステップ０の処理を受け付けました。下限割れor上限越えしている品番をリストアップします。**"},
        ]
        # メッセージを表示
        for message in messages:
            st.chat_message(message["role"]).markdown(message["content"])
    
        st.metric(label="更新日時", value=selected_datetime.strftime("%Y-%m-%d %H:%M"))
        ...
        def calculate_max_consecutive_time(data, time_column, flag_column, group_columns):
            """
        ...
        # 結果をデータフレーム化
        # 関数を使用して計算
        results_df = calculate_max_consecutive_time(
            data=data,  # 入力データ
            time_column='日時',  # 日時列
            flag_column='下限割れ',  # フラグ列
            group_columns=['品番', '受入場所']  # グループ化の基準列
        )
        results_df = results_df.sort_values(by='連続時間（h）', ascending=False).reset_index(drop=True)

        # Streamlitで横並び表示
        col1, col2 = st.columns(2)

        # 下限割れ連続時間が10以上なら赤色を適用する関数
        ...
        with col1:
            st.subheader("📉下限割れ品番リスト")
            styled_df1 = results_df.style.applymap(highlight_red, subset=['連続時間（h）'])
            st.dataframe(styled_df1, use_container_width=True)

        with col2:
            st.subheader("📈上限越え品番リスト")
            styled_df2 = results_df.style.applymap(highlight_red, subset=['連続時間（h）'])
